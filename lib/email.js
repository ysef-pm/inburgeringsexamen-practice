...
// Emails 1-5 — consented nurture sequence (lib/nurture.js builds the content).
function sendNurtureEmail({ to, subject, body, unsubscribeUrl, disclaimer }) {
    const html = `
<div style="font-family:Georgia,'Times New Roman',serif;max-width:36rem;margin:0 auto;color:#1a2332">
  ${body}
  <hr style="border:none;border-top:1px solid #ebe6df;margin:24px 0">
  <p style="font-size:13px;color:#6b7280">${disclaimer}</p>
  <p style="font-size:13px;color:#6b7280">You're receiving preparation tips because you opted in when taking your scorecard.
  <a href="${unsubscribeUrl}" style="color:#6b7280">Unsubscribe</a> — one click, always works.</p>
</div>`;
    return send({ to, subject, html });
}

module.exports = { isConfigured, sendScorecardEmail, sendNurtureEmail };
