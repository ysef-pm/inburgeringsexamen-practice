...
const EVENT_NAMES = new Set([
    ...
    'marketing_optin',
    // flow v2 inline funnel (docs/plans/2026-08-10-mobile-conversion-redesign.md)
    'inline_question_shown', 'inline_answer_submitted', 'signup_sheet_shown',
    'signup_completed', 'first_feedback_viewed', 'checkout_opened',
    'checkout_returned_success', 'checkout_returned_cancelled',
    // flow v3 landing: question-first, email gate after Q3
    'consent_gate_shown',
    // signup instrumentation (2026-08-16): taps vs outcomes per method
    'signup_google_tapped', 'signup_email_tapped', 'signup_failed',
]);
...
// Self-reported comfort is not verified skill: a skill whose points come mostly
// from self-assessment caps below the verified ceiling, so the scorecard can
// never tell someone they are 100% ready off their own claims. (A "94% —
// practice-ready, Speaking 100%" result was talking hot leads out of the
// product two minutes before the AI graded their writing 6/10.)
const SELF_REPORTED_CAP = 80;   // skills graded mostly on self-assessment
const VERIFIED_CAP = 90;        // skills with an equal diagnostic share
function skillCaps() {
    const selfMax = {}; const diagMax = {}; const caps = {}; const selfReported = [];
    for (const skill of rubric.skills) { selfMax[skill] = 0; diagMax[skill] = 0; }
    for (const item of rubric.selfAssessment.items) selfMax[item.skill] += 2;
    for (const q of rubric.diagnostic) diagMax[q.skill] += q.points;
    for (const skill of rubric.skills) {
        const mostlySelf = diagMax[skill] < selfMax[skill];
        caps[skill] = mostlySelf ? SELF_REPORTED_CAP : VERIFIED_CAP;
        if (mostlySelf) selfReported.push(skill);
    }
    return { caps, selfReported };
}
...
function scoreAnswers(answers) {
    ...
    const { caps, selfReported } = skillCaps();
    const skills = {};
    let sum = 0;
    for (const skill of rubric.skills) {
        const pct = Math.min(Math.round((got[skill] / max[skill]) * 100), caps[skill]);
        skills[skill] = pct;
        sum += pct;
    }
    ...
    return { skills, overall, band, focus, selfReported };
}
...
function createScorecardRouter() {
    ...
    // Marketing opt-in from the results screen. The button click is the
    // affirmative GDPR action; the card copy states what they sign up for.
    router.post('/api/scorecard/marketing-opt-in', async (req, res) => {
        const { subscriberId, anonId } = req.body || {};
        if (typeof subscriberId !== 'string' || subscriberId.length === 0 || subscriberId.length > 64) {
            return res.status(400).json({ success: false, error: 'Missing subscriber.' });
        }
        try {
            const db = firebase.getDb();
            const ref = db.collection('subscribers').doc(subscriberId);
            const snap = await ref.get();
            if (!snap.exists) return res.status(400).json({ success: false, error: 'Unknown subscriber.' });

            const utm = cleanUtm(req.body || {});
            await db.collection('consent_events').add({
                subscriber_id: subscriberId, consent_type: 'marketing', action: 'granted',
                notice_version: NOTICE_VERSION, source_page: '/scorecard#results',
                ts: firebase.FieldValue.serverTimestamp(),
            });
            // Re-activates a previously unsubscribed address too: this request
            // is the fresh opt-in the withdrawal comment above requires.
            await ref.set({ subscriber_status: 'active', marketing_consent: true }, { merge: true });
            await logEvent(db, {
                event_name: 'marketing_optin', subscriber_id: subscriberId,
                anon_id: typeof anonId === 'string' ? anonId.slice(0, 64) : null, ...utm,
            });
            res.json({ success: true });
        } catch (err) {
            console.error('scorecard marketing opt-in error:', err);
            res.status(500).json({ success: false, error: 'Could not save your choice. Please try again.' });
        }
    });
    ...
    router.post('/api/scorecard/submit', async (req, res) => {
        ...
        try {
            ...
            const { skills, overall, band, focus, selfReported } = scoreAnswers(answers);
            ...
            // Must complete before res.json(): Vercel freezes the function once the
            // response is sent, so a fire-and-forget send silently dies.
            try {
                ...
                // Email failure must not block the on-screen result.
            ...
            res.json({
                ...
                result: {
                    ...
                    skills, focus, plan, selfReported, disclaimer: rubric.disclaimer,
                },
            });
        } catch (err) {
...
module.exports = { createScorecardRouter, scoreAnswers, signSubscriber };
